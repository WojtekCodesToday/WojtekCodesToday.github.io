roost.extensions["md"] = {
  parseStr(str) {
    ...
    while ((match = regex.exec(str)) !== null) {
      ...
      if (plainText.length > 0) j[`span-${counter++}`] = {
        "child": rawText(plainText)
      };
      if (match[1] === '`') {
        j[`code-${counter++}`] = {
          "child": match[2],
          "class": "roost-md-inline-code"
        };
      } else if (match[3]) {
        j[`strong-${counter++}`] = {
          "child": roost.extensions.md.parseStr(match[4])
        };
      } else if (match[5]) {
        j[`em-${counter++}`] = {
          "child": roost.extensions.md.parseStr(match[6])
        };
      } else if (match[7]) {
        j[`del-${counter++}`] = {
          "child": roost.extensions.md.parseStr(match[8])
        };
      }
      lastIndex = regex.lastIndex;
    }
    const remainingText = str.substring(lastIndex);
    if (remainingText.length > 0) j[`span-${counter++}`] = {
      "child": rawText(remainingText)
    };
  ...
  parse(md = "", options = {}) {
    ...
    while (i < lines.length) {
      ...
      if (trimmed.startsWith(">")) {
        cr("blockquote", {
          "child": parseStr(trimmed.substring(1).trim()),
          "class": "roost-md-quote"
        });
        i++;
        continue;
      }
      ...
      if (hMatch) {
        cr(`h${hMatch[1].length}`, {
          "child": parseStr(hMatch[2]),
          "class": "roost-md-h"
        });
        i++;
        continue;
          ...
          while (i < lines.length) {
            ...
            if (content.startsWith("[ ] ")) {
              liObj.child = {
                ...
                "span-0": {
                  "child": parseStr(content.substring(4))
                }
              };
