function createMangaState() {
  ...
  try {
    ...
    console.warn("Invalid settings payload string parameter.", e);
  }
  var queryEncoder = {
    configs: [],
    add: function (key, getFn, def) {
      this.configs.push({
        key: key,
        getFn: getFn,
        def: def
    ...
    build: function () {
      return this.configs.map(function (c) {
        var val = c.getFn();
        return val !== c.def && val !== null ? c.key + "=" + encodeURIComponent(val) : null;
      }).filter(Boolean).join("&");
    }
  };
  queryEncoder.add("m", function () {
    return manga;
  }, defaultv[0]);
  queryEncoder.add("v", function () {
    return volume;
  }, defaultv[1]);
  queryEncoder.add("c", function () {
    return chapter;
  }, defaultv[1]);
  queryEncoder.add("prov", function () {
    return activeProvider;
  }, null);
  queryEncoder.add("settings", function () {
  ...
  return {
    ...
    set chaptersList(val) {
      chaptersList = val;
    },
    get currentChapterIdx() {
      return currentChapterIdx;
    },
    set currentChapterIdx(val) {
      currentChapterIdx = val;
    },
    get mangaData() {
      return mangaData;
    },
    ...
    addToCache: function (data, images) {
      ...
      if (cache.size > MAX_CACHE_SIZE) {
        ...
        oldest.images.forEach(function (entry) {
          if (entry?.channels) entry.channels.forEach(function (c) {
            if (c) {
              c.width = 0;
              c.height = 0;
            }
  ...
  return {
    resolveBase: async function () {
      ...
      if (state.activeProvider !== null) {
        ...
        throw new Error("Selected provider does not contain manga/chapter: " + providerLabel(selectedRaw) + " -> " + selectedBase);
      }
      ...
      throw new Error("No provider has " + state.manga + "/v" + state.volume + "_c" + state.chapter + ".mps");
    }
  };
}
...
function createMangaRenderer(state, dom) {
  ...
  var executeRenderPass = function () {
    ...
    var vW = dom.container.clientWidth,
      vH = dom.container.clientHeight;
    ...
    if (resizeRequired) {
      dom.canvas.width = vW * dpr;
      dom.canvas.height = vH * dpr;
    ...
    var vY = dom.container.scrollTop,
      vX = dom.container.scrollLeft;
  ...
  return {
    ...
    preparePayload: async function (blobs) {
      var poolMap = new Map();
      await Promise.all(blobs.map(function (b, i) {
        return processAtlasImage(b, i, poolMap);
      }));
      return poolMap;
    },
...
function createInteractionHandler(state, renderer, dom) {
  ...
  return {
    bindEvents: function (onRefreshCall) {
      ...
      window.addEventListener("keydown", function (e) {
        ...
        if (e.key === "ArrowDown" || e.key === "PageDown") {
          e.preventDefault();
          dom.container.scrollTop += dom.container.clientHeight;
          renderer.requestRender();
          if (onRefreshCall) onRefreshCall();
        }
      ...
      dom.container.addEventListener("touchstart", handleDragStart, {
        passive: false
      });
    ...
    setChapterTitle: function (t) {
      activeChapterTitle = t;
    }
  };
}
...
(function () {
  ...
  var computeLayoutGeometry = function () {
    if (!state.mangaData) return;
    var vH = dom.container.clientHeight,
      vW = dom.container.clientWidth;
    var p0 = state.mangaData.p[0];
    var isMangaMode = state.settings.mode === 0;
  ...
  var executeChapterLoad = async function (chTarget) {
    ...
    syncHistoryState();
    state.currentChapterIdx = state.chaptersList.findIndex(function (x) {
      return x.v == state.volume && x.c == state.chapter;
    });
  ...
  var completeEngineRenderInit = function () {
    ...
    renderer.requestRender();
    var currentChapter = state.chaptersList[state.currentChapterIdx] || {
      n: "Unknown",
      v: state.volume,
      c: state.chapter
    };
  ...
  var dispatchLoadPipeline = async function () {
    if (state.chaptersList.length === 0) {
      try {
        ...
        console.error("Navigation tree manifest structural failure.", e);
      }
    }
  ...
  dom.previous.onclick = function () {
    if (state.currentChapterIdx > 0) executeChapterLoad(state.chaptersList[state.currentChapterIdx - 1]);
  };
  dom.next.onclick = function () {
    if (state.currentChapterIdx < state.chaptersList.length - 1) executeChapterLoad(state.chaptersList[state.currentChapterIdx + 1]);
  };
  if (dom.canvas) {
    dom.canvas.addEventListener("layoutChange", computeLayoutGeometry);
  }
