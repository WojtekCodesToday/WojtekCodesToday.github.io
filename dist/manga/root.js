...
export default {
  ...
  css: "/blog.css",
  render: () => {
    return roost.convertJSX(roost.convertJSX, null, roost.convertJSX("style", null, `
                    .search-grid {
                        display: grid;
      ...
      id: "content",
      class: "manga_panel blog_post"
    }, roost.convertJSX("p", {
      style: "padding: 15px;"
    }, "Loading...")), roost.convertJSX("br", null), roost.convertJSX("div", {
      class: "manga_panel"
    }, roost.convertJSX("h2", null, "Q&A:"), roost.convertJSX("ul", null, roost.convertJSX("li", null, "Is this piracy?", roost.convertJSX("ul", null, "No, i make the content myself.", roost.convertJSX("br", null), "thanks for asking though.", roost.convertJSX("br", null), "Also the mangas are provided through a repository/provider.")), roost.convertJSX("br", null), roost.convertJSX("li", null, "Where can i donate to you?", roost.convertJSX("ul", null, "Currently nowhere, i will be thinking of that however :)")), roost.convertJSX("br", null), roost.convertJSX("li", null, "The reader sucks!!", roost.convertJSX("ul", null, "I am aware of that, and i am trying to expand upon it, i know its a bit jank..."))))));
  },
  mount: async () => {
    ...
    try {
      let provRes = await fetch(`manga/providers.json`).catch(() => ({
        json: () => []
      }));
      if (!provRes.ok) provRes = await fetch(`providers.json`).catch(() => ({
        json: () => []
      }));
      ...
      const providerData = await Promise.all(allBaseUrls.map(async (base, pIdx) => {
        try {
          ...
          return {
            provID,
            manifests: manifests.filter(m => m)
          };
        } catch {
          return null;
        }
