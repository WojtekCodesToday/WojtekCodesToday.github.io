...
function clampU8(v) { return clamp(Math.floor(v + 0.5), 0, 255); }
function okread(pos, n, z) { return pos <= z && n <= z - pos; }
function readU32LE(u8, p) { return (u8[p] | (u8[p + 1] << 8) | (u8[p + 2] << 16) | (u8[p + 3] << 24)) >>> 0; }
function readU32BE(u8, p) { return (((u8[p] << 24) >>> 0) | (u8[p + 1] << 16) | (u8[p + 2] << 8) | u8[p + 3]) >>> 0; }
function decodeUtf8(u8) { return new TextDecoder().decode(u8); }
function whiteRgb(w, h) { var rgb = new Uint8Array(w * h * 3); rgb.fill(255); return rgb; }
...
function buildHuffDec(bits, vals) {
    var t = { mincode: new Int32Array(17), maxcode: new Int32Array(17), valptr: new Int32Array(17), vals: vals };
    var code = 0, k = 0;
    for (var i = 0; i <= 16; i++) t.maxcode[i] = -1;
    for (i = 1; i <= 16; i++) {
        if (bits[i] === 0) { code <<= 1; continue; }
        t.valptr[i] = k;
        t.mincode[i] = code;
        code += bits[i];
        k += bits[i];
...
BitReader.prototype.nextByte = function() { return this.pos < this.data.length ? this.data[this.pos++] : 0; };
...
function decodeMpsPrivateJpeg(data, gray, w, h) {
    ...
    var bwid = Math.ceil(w / 8), bhei = Math.ceil(h / 8), planew = bwid * 8, planeh = bhei * 8;
    var planes = [];
    ...
    return { rgb: rgb, consumed: br.pos };
}
...
function rgbToPngBlob(rgb, w, h) {
    ...
    var png = concatArrays([
        new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]),
        pngChunk("IHDR", ihdr),
        pngChunk("IDAT", pako.deflate(raw)),
...
function parseMpsFile(buffer) {
    ...
    return { data: { title: title, ptype: ptype, p: pages }, blobs: blobs };
}
...
function parseLegacyBin(buffer) {
    ...
    if (data.p) {
        data.p = data.p.map(function(p) {
            return { w: p[0], h: p[1], atlasIdx: p[2], chunks: p[3] };
        });
    }
